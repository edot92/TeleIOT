  ...
  export default {
    ...
    methods: {
      ...
      getlist () {
        ...
        thisV.$store.dispatch('User/httpPetugasGetList', {
          offset: offset,
          limit: limit
        }).then(value => {
          if (value.error === false) {
            if (value.payload.totalItems <= thisV.pagination.rowsPerPage) {
              value.payload.totalItems = thisV.pagination.rowsPerPage
            }
            thisV.totalItems = value.payload.totalItems
            thisV.items = []
            thisV.items = value.payload.user
            thisV.indexNo = []
            let count = offset + 1
